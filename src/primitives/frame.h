...
struct Frame {
    ...
    static float sinPhi(const Vector3f &v) {
        ...
        if (sinTheta == 0.0f)
            return 1.0f;
        return std::clamp(v.y() / sinTheta, -1.0f, 1.0f);
    }
    ...
    static float cosPhi(const Vector3f &v) {
        ...
        if (sinTheta == 0.0f)
            return 1.0f;
        return std::clamp(v.x() / sinTheta, -1.0f, 1.0f);
    }
    ...
    static float sinPhi2(const Vector3f &v) {
        return std::clamp(v.y() * v.y() / sinTheta2(v), 0.0f, 1.0f);
    }
    ...
    static float cosPhi2(const Vector3f &v) {
        return std::clamp(v.x() * v.x() / sinTheta2(v), 0.0f, 1.0f);
    }
