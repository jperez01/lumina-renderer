...
ImageBlock::ImageBlock(const Vector2i &size, const ReconstructionFilter *filter)
    ...
    if (filter) {
        ...
        m_filter = new float[LUMINA_FILTER_RESOLUTION + 1];
        for (int i=0; i<LUMINA_FILTER_RESOLUTION; ++i) {
            float pos = (m_filterRadius * i) / LUMINA_FILTER_RESOLUTION;
            m_filter[i] = filter->eval(pos);
        }
        m_filter[LUMINA_FILTER_RESOLUTION] = 0.0f;
        m_lookupFactor = LUMINA_FILTER_RESOLUTION / m_filterRadius;
...
void ImageBlock::fromBitmap(const Bitmap &bitmap) {
    if (bitmap.cols() != cols() || bitmap.rows() != rows())
        throw LuminaException("Invalid bitmap dimensions!");
...
void ImageBlock::put(const Point2f &_pos, const Color3f &value) {
    if (!value.isValid()) {
        ...
        std::cerr << "Integrator: computed an invalid radiance value: " << value.toString() << "\n";
        return;
    }
